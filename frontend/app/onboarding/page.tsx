    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-white/50 leading-relaxed">
          Your hiring agent needs an on-chain identity (ERC-8004) — a wallet address, reputation record, and a name workers can bid on.
        </p>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-white/50 leading-relaxed">
          Scouts evaluate incoming bids by different strategies. You can override per task.
        </p>
      </div>
        ...
        {SCOUTS.map(scout => {
          ...
          return (
            <button
              ...
              className={`text-left flex items-center gap-4 p-4 rounded-xl border transition-all duration-150 ${
                active ? "border-white/25 bg-white/[0.06]" : "border-white/[0.10] bg-[#0f0f0d] hover:border-white/[0.18]"
              }`}
            >
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-white/50 leading-relaxed">
          Your agent gets a one-time permission to spend only up to your set budget — only for that task, only for one hour.
        </p>
      </div>
...
export default function Onboarding() {
  ...
  const next = () => {
    if (isLast) { localStorage.setItem("hivebid_onboarded", "1"); router.push("/dashboard") }
    else setStep(s => s + 1)
  }
